...
class SidebarBridge(QObject):
    ...
    def _on_watcher_refresh(self):
        ...
        if self._suppress_refresh or getattr(self, '_suppress_watcher', False) or not self._project_path:
            return
    ...
    def listDirectory(self, dir_path: str) -> str:
        ...
        try:
            ...
            for item in sorted(path.iterdir(), key=lambda x: (not x.is_dir(), x.name.lower())):
                # Only skip .git directory — everything else shows in the file tree.
                # Build/cache dirs (dist, build, node_modules, etc.) are visible
                # so users can browse them. Search section filters them separately.
                if item.name == '.git':
                    continue
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().split('\n'):
                        ...
                        # Get line diff stats for this file
                        additions = 0
                        deletions = 0
                        try:
    ...
    def refreshGitStatus(self, _path=None):
        ...
        try:
            # Invalidate getGitStatus cache so next call fetches fresh data
