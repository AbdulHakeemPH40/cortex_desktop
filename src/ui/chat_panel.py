...
from PyQt6.QtCore import Qt, QObject, pyqtSignal, QThread, QTimer, QPropertyAnimation, QEasingCurve, QSize, QEvent
...
from src.ui.tokens import DARK as T, build_qss, build_markdown_css, build_code_block_css
...
_STREAM_HL_CACHE: dict[tuple[str, str], str] = {}
...
def _extract_code_fences_for_streaming(text: str) -> tuple[str, dict[str, str]]:
    ...
    def _stash(m):
        ...
        lang_label = f'<span style="color:#7c5cff;font-size:11px;">{_html.escape(lang)}</span><br/>' if lang else ''
        styled = (f'<pre style="margin:8px 0;padding:10px 14px;background:#1e1e1e;'
                  f'border:1px solid #3c3c3c;border-left:3px solid #7c5cff;'
...
def _fix_prose_tables(html: str, doc_width: int = 760) -> str:
    ...
    def _fix_one_table(m):
        ...
        col_border = '#353535'
        header_style = (
            f'color:#9d7cd8;font-weight:600;padding:8px 10px;'
            f'text-align:left;vertical-align:top;'
            f'border-bottom:2px solid #6a568e;'
            f'border-right:1px solid {col_border};'
            f'width:{col_w};'
            f'word-wrap:break-word;'
        )
        data_style = (
            f'color:#d9d9d9;padding:8px 10px;'
            f'text-align:left;vertical-align:top;'
            f'border-bottom:1px solid #353535;'
            f'border-right:1px solid {col_border};'
            f'width:{col_w};'
            f'word-wrap:break-word;'
        )
...
_CARD_FRAME_DEFAULT = _card_frame_qss()
_CARD_FRAME_SUBTLE = _card_frame_qss(T['card_border_subtle'], T['card_bg_subtle'])
        ...
        if 'matches' in data and data['matches']:
            ...
            for m in data['matches'][:20]:
                ...
                if f and l:
                    ...
                    lines.append(f)
            match_count = data.get('match_count', len(data['matches']))
...
class TableWidget(QFrame):
    ...
    def __init__(self, headers: list[str], rows: list[list[str]], parent=None):
        ...
        # QSS — opencode style:
        # headers: purple, bold, bottom border only
        # cells: bottom border only + right border for vertical separators
        # no outer left/right borders on first/last columns
        # Outer frame border — visible white border around entire table
        self.setStyleSheet(
            "QFrame { border: 1px solid #464646; margin: 4px 0; }"
        )
        ...
        table.setStyleSheet(
            ...
            f"  border: none;"
            f"  border-bottom: 1px solid #353535;"
            f"  border-right: 1px solid #353535;"
            f"}}"
        ...
        for r, row_data in enumerate(rows):
            ...
            for c, cell_text in enumerate(padded[:n_cols]):
                ...
                if is_bold:
                    item.setForeground(_QColor("#ebebeb"))
...
class _ChatBrowser(QTextBrowser):
    ...
    def contextMenuEvent(self, e):
        ...
        # Tint all action icons white so they're visible on dark bg
        for action in menu.actions():
            ic = action.icon()
            if not ic.isNull():
                action.setIcon(_white_icon(ic))
        menu.setStyleSheet(
            f"QMenu {{ background:{T['context_menu_bg']}; color:{T['white']}; border:1px solid {T['context_menu_border']};"
            f"  border-radius:6px; padding:4px; }}"
            f"QMenu::item {{ color:{T['white']}; padding:5px 24px; border-radius:4px; }}"
            f"QMenu::item:selected {{ background:{T['context_menu_sel']}; color:{T['white']}; }}"
            f"QMenu::separator {{ height:1px; background:{T['context_menu_border']}; margin:4px 8px; }}"
        )
        menu.exec(e.globalPos())


_white_icon_cache: dict[int, QIcon] = {}

def _white_icon(icon, size=16):
    """Re-color any QIcon to solid white (preserves alpha shape). Cached.
    ...
    # Tint the hi-res pixmap solid white (keep alpha mask)
    out = QPixmap(hi, hi)
    ...
    result = QIcon(final)
    _white_icon_cache[cache_key] = result
    return result
...
class SpellCheckInput(QTextEdit):
    ...
    def contextMenuEvent(self, e):
        _MENU_STYLE = (
            f"QMenu {{ background:{T['spell_input_bg']}; color:{T['white']}; border:1px solid {T['separator']};"
            f"  border-radius:8px; padding:4px; font-size:13px; }}"
            f"QMenu::item {{ padding:6px 20px; border-radius:4px; }}"
            f"QMenu::item:selected {{ background:{T['btn_hover']}; color:{T['white']}; }}"
...
class InputArea(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.send_btn.setStyleSheet(
            ...
            f"QPushButton:hover {{ background:{T['btn_hover']}; color:{T['white']}; }}"
        )
...
# ============================================================
# 6c. EMPTY STATE — animated ring + tagline
# ============================================================
class CortexRing(QWidget):
    """Animated rotating ring — the Cortex neural loop icon."""
    ...
    def _tick(self):
        self._angle = (self._angle + 1.44) % 360  # 360°/25s at 30fps
        self._breath += 0.005 * self._breath_dir
        if self._breath >= 0.90:
            self._breath_dir = -1
        elif self._breath <= 0.65:
            self._breath_dir = 1
        self.update()
    ...
    def paintEvent(self, event):
        from PyQt6.QtGui import QPainter, QConicalGradient, QPen, QColor, QRadialGradient
...
class EmptyState(QWidget):
    """Empty chat state: animated ring + tagline."""
    ...
    def __init__(self, parent=None):
        ...
        self.ring = CortexRing(80)
        v.addWidget(self.ring, alignment=Qt.AlignmentFlag.AlignCenter)

        title = QLabel("Start a new conversation with Cortex AI")
        title.setStyleSheet(
        ...
        sub = QLabel("Ask, plan, or build — the AI is ready.")
        sub.setStyleSheet(
...
class BrandLogo(QWidget):
    ...
    "AI"      — same font, weight 400, accent cyan (#06b6d4), slightly smaller,
...
class ChatPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._header.setStyleSheet("background: transparent; border-bottom: 1px solid #1E2D3D;")
        ...
        self._new_chat_btn.setStyleSheet(
            "QPushButton { color:#8B9CC7; font-size:12px; font-weight:500;"
            "padding:5px 14px; border-radius:6px; border:1px solid #2A3A4A;"
            "background:#151D2B; font-family:'Segoe UI',sans-serif; }"
            "QPushButton:hover { background:#1E2D3D; color:#5B8CFF; border-color:#5B8CFF; }"
        )
        ...
        # Safety net: set viewport background to app dark so any repaint
        # that bypasses freeze/thaw shows dark instead of white (Windows default).
        self.scroll.viewport().setStyleSheet("background: #1e1e1e;")
        self.container = QWidget()
        self.container.setStyleSheet("background: #1e1e1e;")  # Prevent white flash during thaw
        ...
        self._footer.setStyleSheet(
            "QWidget { background:transparent; border-top:1px solid #1E2D3D; }"
        )
    ...
    def on_tool_end(self, tool_id, status, result_data=None):
        ...
        with self._stabilize_scroll():
            ...
            # Force new block on next text/thinking
    ...
    def _finalize_mermaid_cards(self):
        ...
        _raw_buf = self._prose_buf or ""
        _mermaid_inserted = False
        for _mm in _RE_MERMAID_COMPLETE.finditer(_raw_buf):
            _fence = _mm.group(0)
            _code = re.sub(r'^```mermaid[ \t]*\r?\n', '', _fence, flags=re.IGNORECASE)
            _code = re.sub(r'\n```\s*$', '', _code).strip()
            if _code and _sc_layout is not None and _sc_idx >= 0:
                ...
                _sc_idx += 1
                _mermaid_inserted = True

        # FIX: If no complete mermaid blocks found but streaming card existed,
        # check for an OPEN (incomplete) mermaid block. The AI may have stopped
        # mid-diagram (no closing ```). Extract whatever code is available and
        # create a diagram card from it instead of silently losing it.
        if not _mermaid_inserted:
            _open_mm = _RE_MERMAID_OPEN.search(_raw_buf)
            if _open_mm:
                _open_fence = _open_mm.group(0)
                _code = re.sub(r'^```mermaid[ \t]*\r?\n', '', _open_fence, flags=re.IGNORECASE).strip()
                if _code and _sc_layout is not None and _sc_idx >= 0:
        ...
        if _mermaid_inserted:
            self._prose_buf = _RE_MERMAID_COMPLETE.sub('', self._prose_buf)
            self._prose_buf = _RE_MERMAID_OPEN.sub('', self._prose_buf).strip()

    def _collect_last_prose_block(self):
        if self._open_kind == "prose" and self._open_block and self._prose_buf.strip():
            self._prose_blocks.append((self._open_block, self._prose_buf))
            self._prose_buf = ""
    ...
    def _refit_all_bodies(self, prebuilt: list | None = None):
        ...
        def _refit_batch(start: int):
            ...
            if end < total:
                ...
                log.info(f"[ChatRestore] Refit complete: {total} widgets processed")
    ...
    def load_chat(self, conversation_id: str):
        ...
        store = ChatStore()
        # Try to get DB from parent (main_window sets it)
        db = getattr(self, '_db', None)
        if db:
            store.set_db(db)
        turns = store.load_chat(conversation_id)
        if not turns:
            return
        # Clear existing messages
        self.clear_messages()
        # Rebuild from timeline
        for turn in turns:
    ...
    def set_theme(self, is_dark: bool):
        ...
        for tb in self.findChildren(ThoughtsBlock):
            try:
                tb._apply_theme_styles()
                tb._refresh_body_style()
            except RuntimeError:
