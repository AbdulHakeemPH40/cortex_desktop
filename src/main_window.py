...
class CortexMainWindow(QMainWindow):
    ...
    def _apply_dark_title_bar(self):
        """Apply dark native Windows title bar via DWM API.
        ...
        """
        if getattr(self, '_dark_title_applied', False):
            return
        try:
            hwnd = int(self.winId())
            if not hwnd:
            ...
            log.info(f"[DWM] Dark title bar applied to HWND={hwnd}")
        except Exception as e:
            log.warning(f"[DWM] Dark title bar failed: {e}")
    ...
    def _build_ui(self):
        ...
        main_splitter.setSizes([240, 500, 500])
        ...
        self._left_sidebar_min_width = 240
        ...
        # ════════════════════════════════════════════════════════════════
        # FULL-WINDOW STARTUP OVERLAY — covers entire window during init
        # ════════════════════════════════════════════════════════════════
        # Previously only the SidebarWidget had a loading overlay, leaving
        # the chat panel and editor panel visible but empty during startup.
        # This caused the 3-step visual glitch:
        #   Step 1: Partial content (sidebar overlay covers only sidebar)
        #   Step 2: Dark flash during maximize (uncovered panels repaint)
        #   Step 3: Full load when sidebar.html finishes
        # NOTE: _startup_overlay removed — caused DWM capsule fragments when
        # overlapping with sidebar._loading_overlay and sidebar.html splash.
        # Sidebar has its own loading overlay + HTML splash. No need for a
        # third full-window QWidget overlay.
    ...
    def _on_project_opened(self, folder_path: str):
        ...
        _is_generating = getattr(getattr(self, '_ai_agent', None), '_is_generating', False)
        if _is_generating:
            log.warning("[PROJECT-SWITCH] AI is mid-turn — cancelling before switch")
            try:
                self._ai_agent.cancel()
            except Exception:
                pass
    ...
    def _emergency_shutdown_save(self) -> None:
        ...
        try:
            ...
            if bridge is not None:
                ...
                try:
                    bridge.save_session_to_memory()
                    log.info("[SHUTDOWN] MEMORY.md saved on shutdown")
                except Exception as _mem_exc:
    ...
    def _on_ai_task_complete(self, response: str):
        ...
        Respects settings: notifications.task_complete_enabled, notifications.only_when_unfocused.
        """
        try:
